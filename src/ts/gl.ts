...
export namespace gl
{
  export function getGLContext(canvas: HTMLCanvasElement): WebGLRenderingContext
  {
    width = canvas.width;
    ...
    if (!context) throw `Unable to get GL context.`;
    return context;
  }

  export function initGL(context: WebGLRenderingContext): void
  {
    ctx = context;
    ...
    function compileShader(source: string, type: number): WebGLShader
    {
      ...
      if (!glShader) throw `Unable to created shader`;
    ...
    function createShaderProgram(vsSource: string, fsSource: string): WebGLProgram
    {
      ...
      if (!program) throw `Unable to created program`;
    ...
    function createBuffer(bufferType: number, size: number, usage: number): WebGLBuffer
    {
      ...
      if (!buffer) throw `Unable to created buffer`;
